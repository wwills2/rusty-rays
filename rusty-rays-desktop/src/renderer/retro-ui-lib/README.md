...
- run ```npm run ui-add <component>``` to install a new retro ui component
    - ```npm run ui-add button``` to install the button
- About the aforementioned errors:
    ...
    - Components that the installed components depend on may have not been added yet. These components will also need to
      be
